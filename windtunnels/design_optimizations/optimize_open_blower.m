        ...
        for A2 = A2_range
            for A3 = A3_range
                for L3 = L3_range
                    ...
                    parts = add_part(wind_tunnel, parts, 'contraction'          ,'contraction'                    , sqrt(A1), sqrt(A1), 0.8, 0.8                , 'shape' ,'curved', 'center_length', 1.5); % length = length along the center line
                    % blower fan
                    parts = add_part(wind_tunnel, parts, 'Diffuser'              ,'diffuser'                      , 0.8, 0.8,  sqrt(A2), sqrt(A2)               , 'cross_section_shape', 'rectangle', 'length_center_line', L2, 'length_wall', 0); % Only fill in length_center_line OR length_wall, but NOT both!
                    ...
                    parts = add_part(wind_tunnel, parts, 'Rectangular diffuser'  ,'diffuser'                      , 1.5, 1.07, sqrt(A3), sqrt(A3)           , 'cross_section_shape', 'rectangle', 'length_center_line', L3, 'length_wall', 0); % Only fill in length_center_line OR length_wall, but NOT both!
...
Title      = 'Blower Wind tunnel';
X_label    = 'Cross section Settling chamber(m^2)';
Y_label    = 'Fan power (W)';
Y_min      = 25000;     % if you do not want to use it, set it to: NaN
Y_max      = 50000;     % if you do not want to use it, set it to: NaN
line_size  = 2;
text_size  = 2;
line_legend   = 'Max fan power (35kW)'; % You should only specify 1 line here
plot_legend_1 = 'Fan power for L2 = ';
plot_legend_2 = 'm';
...
% Plot the outcome values
Y_line     = 35000; % if you do not want to use it, set it to: NaN
Title      = 'Blower Wind tunnel (index plot)';
