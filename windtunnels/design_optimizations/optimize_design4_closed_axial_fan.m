% example_optimise_script - closed ciruit WT, thesis chapter 5.2.4, Fig. 5.15
%
...
for A3 = A3_range 
     for A1 = A1_range
         for A2 = A2_range
                if A1 > A2  % if A2>A1 --> error in pretty plot (index plot does work)!!
                    continue;
                end
                
                parts = [];
                parts = add_part(wind_tunnel, parts, 'Rectangular diffuser'  ,'diffuser'                      , 1.25, 1.25, sqrt(A1), sqrt(A1),             'cross_section_shape', 'rectangle', 'length_center_line', 11.5, 'length_wall', 0); % Only fill in length_center_line OR length_wall, but NOT both!
                        ...
                        if (tmp_part.angle_degrees > 10)
                         ...
                         if (tmp_part.angle_degrees > 15)
                              ...
                              if (tmp_part.angle_degrees > 20)
                                arts = add_part(wind_tunnel, parts, 'BLC Screen 1' , 'screen'                         , sqrt(A1), sqrt(A1), sqrt(A1), sqrt(A1),'wire_diameter', 0.001, 'wire_roughness', 1.3, 'cell_diameter', 0.01);
                                end
                ...
                parts = add_part(wind_tunnel, parts, 'Curved contraction'    ,'contraction'                   , sqrt(A2), sqrt(A2), 1.5, 2 ,                    'shape' ,'curved', 'center_length', 1.3); % length = length along the center line
                ...
                parts = add_part(wind_tunnel, parts, 'Curved contraction'    ,'contraction'                   , sqrt(A3), sqrt(A3), 1.25, 1.25   ,              'shape' ,'curved', 'center_length', 0.5); % length = length along the center line
...
X_label    = 'A2 (m)';
Y_label    = 'Fan power (W)';
Y_min      = 24000;     % if you do not want to use it, set it to: NaN
Y_max      = 35000;     % if you do not want to use it, set it to: NaN
line_size  = 2;
text_size  = 4;
titel_plot{1} = '';
titel_plot{2} = 'A2 = 13 m';
titel_plot{3} = 'A2 = 16 m';
