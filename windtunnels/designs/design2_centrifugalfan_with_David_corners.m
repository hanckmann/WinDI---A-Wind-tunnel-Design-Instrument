...
wind_tunnel.efficiency_fan      = 0.6;
...
A1_range = 1.56; %corners downstream the fan, reused from David
%A1_range = [2 : 0.1 : 4];

A2_range = 5; %Settling chamber cross section, reused from David
%A2_range = [5: 3 : 11];

%A3_range = 1.68; % Corner downstream the test section reused from david
%A3_range = [ 1.7 : 0.1 : 4 ];

%L1_range = [0.5: 0.1 : 4];
L1_range = 3; %diffuser length between corner and settling chamber
    ...
    for A2 = A2_range
        for L1 = L1_range
            for A1 = A1_range
                ...
                parts = add_part(wind_tunnel, parts, 'Straight contraction'  , 'contraction'                    ,  1.08, 1.5, .6, .89,                      'shape' ,'straight', 'center_length', 0.5); % length = length along the center line
                parts = add_part(wind_tunnel, parts, 'Corner'                , 'corner_downstream_testsection'  ,  .6, .85, .6, .92,                        'chord', .42);
                parts = add_part(wind_tunnel, parts, 'Straight contraction'  , 'contraction'                    ,  .6, .92, .51, .51 ,                      'shape' ,'straight', 'center_length', 1.1); % length = length along the center line%remore added inlet contraction for this design!!
